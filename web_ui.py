...
def _scheduler_loop():
    """后台调度器：北京时间 7:00-20:00，随机间隔 60-90min，每账号随机 3-5 个。多账号轮询 + 自动重登。"""
    import random as _random
    from icloud_hme import ICloudHME
    _update_state(running=True, round_status="等待触发窗口")
    _emit_log("info", "Scheduler started (BJ 7-20h, 间隔 60-90min, 每轮 3-5 个/账号, 多账号轮询)")
    def _bj_hour() -> int: return (_now().hour + 8) % 24
    def _safe_client(account):
        """构造 client；若 cookie stale 则尝试自动重登（用密码）。"""
        try:
            ...
            return None
    # 每轮开始前同步一次 pool session cookie（避免 cookie stale）
    while not _stop_event.is_set():
        try:
            upd = _account_mgr.refresh_cookies_from_pool()
            if upd:
                _emit_log("info", f"同步 {len(upd)} 个账号 cookie 自 pool session")
        except Exception:
            pass
        ...
        if not active_accounts: _update_state(creating=False, round_status="无Active账号，跳过"); _stop_event.wait(1800); continue
        round_total = 0
        for i, account in enumerate(active_accounts):
            ...
            if client is None:
                _emit_log("warn", f"[{acc_name}] Cookie失效，尝试自动重登...")
                try:
                    _account_mgr.validate_account(acc_id)
                except Exception as e:
                    _emit_log("error", f"[{acc_name}] 自动重登失败: {str(e)[:80]}")
                continue
            created = 0; errors = 0
            while created < target_count and errors < 3 and not _stop_event.is_set():
                try:
                    ...
                    # persistent log untuk debug
                    try:
                        with open(str(LOGS_DIR/"scheduler_errors.log"),"a",encoding="utf-8") as f:
                            f.write(f"{_now().isoformat()}\t{acc_name}\t{err_str[:300]}\n")
...
def api_create_batch():
    ...
    _update_state(creating=True)
    _emit_log("info",f"Batch Create: {len(account_ids)} 个账号 x{count}")
    try:
